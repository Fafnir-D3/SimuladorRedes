...
# Duração do slot em segundos. O valor padrão de 150 ms foi escolhido
# com base nos valores padrão do tamanho do pacote (1500 bytes) e taxa 
# de transmissão (100 kb/s). Com esses valores, o atraso de transmissão
# é de 120 ms. Assim, um slot de 150 ms é suficiente para a transmissão 
# e mais até 30 ms de propagação.
duracaoSlot = 0.150

# Probabilidade de tentar uma retransmissão em um determinado slot. O 
# valor padrão de 0.5 foi escolhido por ser o ideal para dois transmissores.
prob = 0.5
...
class Interface:
    ...
    # pela camada de enlace para transmissão.
    def MACInicializa(self, p):
        ...
        # Quando o pacote é inicialmente recebido para transmissão, devemos aguardar o 
        # início do próximo slot e transmitir. 
        # 
        # Vamos calcular quanto tempo falta para o início do próximo slot.
        proximoSlot = math.ceil(self.escalonador.getAgora() / duracaoSlot) * duracaoSlot - self.escalonador.getAgora()

        # Vamos usar a rotina de backoff para esperar o tempo necessário.
        # Vamos usar a propriedade 'retransmissao' para indicar para o método de finalização
        # do backoff que se trata da primeira tentativa de transmissão do pacote (e não de uma
        # retransmissão).
        self.retransmissao = 0 
        self.MACExecutaBackoff(proximoSlot)
    ...
    def MACFinalizaBackoff(self, p):
        ...
        # Essa é a primeira tentativa de transmissão de p?
        if self.retransmissao == 0:

            # Sim, fizemos um backoff apenas para esperar o início do 
            # próximo slot. Simplesmente transmitimos.
            self.MACTransmitePacote(p)
        else:

            # Não, estamos em um backoff de fato. Precisamos reavaliar 
            # se devemos tentar a retransmissão nesse slot.
            if random.random() < prob:

                # Probabilisticamente, decidimos retransmitir.
                self.MACTransmitePacote(p)
            else:
                # Probabilisticamente, decidimos esperar mais um slot.
                self.MACExecutaBackoff(duracaoSlot)

    # Método que é chamado quando a última tentativa de transmissão realizada (iniciado 
    ...
    def MACFinalizaTentativaDeTransmissaoMalSucedida(self, p):
        ...
        # Quando uma tentativa de transmissão falha, devemos esperar até o
        # início do próximo slot para tentar uma nova transmissão. 
        # 
        # Vamos calcular quanto tempo falta para o início do próximo slot.
        proximoSlot = math.ceil(self.escalonador.getAgora() / duracaoSlot) * duracaoSlot - self.escalonador.getAgora()
        ...
        # Vamos usar a rotina de backoff para esperar o tempo necessário.
        # Vamos usar a propriedade 'retransmissao' para indicar para o método de finalização
        # do backoff que se trata de uma retransmissão do pacote (e não da primeira
        # tentativa).
        self.retransmissao = 1
        self.MACExecutaBackoff(proximoSlot)
...
def leArgumentos():
    ...
    parser.add_argument('pos', type=float, default=[0, 10000000000, 10000000000, 10000000000, 10000000000, 10000000000], nargs='*',
